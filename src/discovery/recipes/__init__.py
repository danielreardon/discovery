...
def ecorr_correlated(psr):
    """Correlated-mode Legendre ECORR -> dense 2D variable prior covariance
    (``NoiseMatrix2D_var`` via ``makegp_ecorr_legendre_correlated``)."""
    return ds.PulsarLikelihood([
        psr.residuals,
        ds.makenoise_measurement(psr, psr.noisedict),
        ds.makegp_timing(psr, svd=True),
        ds.makegp_ecorr_legendre_correlated(psr, nmodes=3),
    ])
