...
def makegp_timedomain_solar_dm(psr, covariance, dt=1.0, common=[], name='timedomain_sw_gp'):
    ...
    The TOAs are quantized into time bins, and the GP is constructed using the time
    separations between bins weighted by the solar wind DM signature.
    ...
    :class:`matrix.VariableGP`
        ...
        to the TOA residuals via solar wind DM delays.
    """
    ...
    n_coeffs = Umat.shape[1]
    gp.index = {f'{psr.name}_{name}_coefficients({n_coeffs})': slice(0, n_coeffs)}
